	...
	buildMenu: function buildMenu() {
		...
		if (this.props.allowCreate && this.state.inputValue.trim()) {
			...
			if(options.indexOf(newOption) !== -1){
				...
				console.log(options.indexOf(newOption))
		...
		if (this.props.multi) {
			this.state.values.forEach(function (val) {
				if(this.state.values.indexOf(val) == -1){
					console.log("here it is. add it")
				}
				else{
					console.log("here it is, don't add it");
				}
				...
				for(var key in this.state.values){
					console.log(this.state.values[key].label);
					if(this.state.values[key].label == val.value)
						test = key;
					console.log("actually equal");
				}
				console.log(test);
				if (this.state.values.indexOf(val.label) !== -1){
					console.log(typeof(val.label));
					console.log(this.state.values);
					console.log(this.state.values.indexOf(val.label));
					console.log("doesn't = -1");
				}
				else{
					console.log(typeof(val.label));
